...
export async function applyDynamicTheme(config,imageOverride=null,opts={}){
  ...
  const hero=theme.heroCarousel||{},heroImage=Array.isArray(hero.images)&&hero.images[0]?hero.images[0]:"";
  const image=imageOverride||bg.image||heroImage||"";
  const fallback=theme.seedColor||"#6750A4";
  const manual=theme.source==="manual"?fallback:"";
  const heroColor=theme.source==="hero"?(hero.colors?.[heroImage]||""):"";
  const seed=manual||heroColor||(theme.dynamicColor!==false?await extractSeed(image,fallback):fallback);
