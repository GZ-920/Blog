...
function renderHeroList(images){
  ...
  box.querySelectorAll("[data-hero-pick-color]").forEach(btn=>btn.onclick=()=>btn.closest(".hero-image-item")?.querySelector("[data-hero-color]")?.click());
}
...
async function uploadHeroFiles(){
  ...
  try{
    ...
    if(btn)btn.disabled=true;const existing=readHeroList();let next=1;const files=[];
...
async function githubFetch(url,options={},action="GitHub 请求"){
  let r;try{r=await fetch(url,{...options,headers:{...authHeaders(),...(options.headers||{})}});}catch(e){throw new Error(`${action}失败：${e?.message||"网络请求失败（可能是浏览器网络/CORS拦截）"}`);}
...
async function githubCommit(files,message){
  ...
  const refUrl=repoApi(`git/ref/heads/${gh.branch}`);
  for(let attempt=0;attempt<3;attempt++){
    try{
      const ref=await githubFetch(refUrl,{},"读取分支");
      ...
      try{
        await githubFetch(refUrl,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({sha:newSha,force:false})},"更新分支");
        return newSha;
      }catch(e){
