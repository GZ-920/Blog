...
let config={},posts=[],current=null,currentMd="",gh={owner:"",repo:"",branch:"main",token:""},pendingHeroFiles=[];
...
function bindConfig(){
  ...
  const heroFile=$("[data-hero-file]");
  const heroUploadBtn=$("[data-hero-upload-submit]");
  if(heroFile)heroFile.onchange=()=>{
    pendingHeroFiles=[...heroFile.files];
    const state=$("[data-hero-upload-state]");
    if(state)state.textContent=pendingHeroFiles.length?`已选择 ${pendingHeroFiles.length} 张图片`:"";
    if(heroUploadBtn)heroUploadBtn.disabled=!pendingHeroFiles.length||!gh.token;
  };
  if(heroUploadBtn)heroUploadBtn.onclick=()=>handleHeroUpload().catch(e=>toast(e.message,9000));
}
...
async function handleHeroUpload(){
  const files=pendingHeroFiles.length?pendingHeroFiles:[...($("[data-hero-file]")?.files||[])];
  if(!files.length){toast("请先选择头图");return;}
  if(!gh.token){toast("请先在 GitHub 页面测试连接");return;}
  const state=$("[data-hero-upload-state]"),btn=$("[data-hero-upload-submit]");
  if(btn)btn.disabled=true;
  if(state)state.textContent="上传中…";
  try{
    const names=await listImageNames();let n=nextHeadNumber(names),list=heroItems();
    for(const file of files){
      const ext=imageExtension(file);let name;
      do{name=`head${String(n).padStart(2,"0")}.${ext}`;n++;}while(names.includes(name));
      const path=`images/${name}`,color=await extractImageSeed(file);
      await githubWriteBinary(path,file,`Add hero image: ${path}`);
      list.push({src:path,color});names.push(name);
    }
    renderHeroList(list);
    config=collectConfig();
    await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update hero carousel");
    pendingHeroFiles=[];
    const input=$("[data-hero-file]");if(input)input.value="";
    if(state)state.textContent=`已上传 ${files.length} 张`;
    $("[data-hero-state]").textContent="已保存";
    toast(`头图已上传 ${files.length} 张并保存`);
  }catch(e){
