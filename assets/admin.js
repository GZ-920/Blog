...
let config={},posts=[],current=null,currentMd="",pendingHero=[],gh={owner:"",repo:"",branch:"main",token:""};
...
function fillEditor(){if(!current)return;$("[data-editor-heading]").textContent=current.title||"新文章";$("[data-post-file]").value=current.file||"";$("[data-post-cat]").value=current.cat||"essay";$("[data-post-title]").value=current.title||"";$("[data-post-desc]").value=current.desc||"";$("[data-post-category]").value=current.category||"";$("[data-post-date]").value=current.date||new Date().toISOString().slice(0,10);$("[data-post-time]").value=current.time||"5 min";const cover=current.coverHome||current.coverArticle||current.cover||"";const coverLegacy=$("[data-post-cover]"),coverHomeEl=$("[data-post-cover-home]"),coverArtEl=$("[data-post-cover-article]");if(coverHomeEl)coverHomeEl.value=current.coverHome||current.cover||"";if(coverArtEl)coverArtEl.value=current.coverArticle||current.cover||"";if(coverLegacy)coverLegacy.value=cover;$("[data-post-featured]").checked=!!current.featured;$("[data-post-tags]").value=asList(current.tags).join(", ");$("[data-markdown]").value=currentMd;renderPreview();}
function collectPost(){const home=$("[data-post-cover-home]")?.value.trim()||"",art=$("[data-post-cover-article]")?.value.trim()||"",legacyField=$("[data-post-cover]")?.value.trim()||"";return{file:$("[data-post-file]").value.trim(),cat:$("[data-post-cat]").value,category:$("[data-post-category]").value.trim(),log:current?.log||"POST",title:$("[data-post-title]").value.trim(),desc:$("[data-post-desc]").value.trim(),date:$("[data-post-date]").value,time:$("[data-post-time]").value.trim(),coverHome:home,coverArticle:art,cover:home||art||legacyField,featured:$("[data-post-featured]").checked,tags:asList($("[data-post-tags]").value)};}
