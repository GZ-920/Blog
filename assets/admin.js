...
function collectConfig(){
  ...
  const hero=readHeroData();out.theme={...(out.theme||{}),heroCarousel:{enabled:$("[data-hero-enabled]").checked,autoplay:$("[data-hero-autoplay]").checked,interval:Math.max(1000,Number($("[data-hero-interval]").value)||3000),images:hero.images,colors:hero.colors}};
...
function bindConfig(){const reset=$("[data-reset-config]");if(reset)reset.onclick=async()=>{try{config=await loadConfig(true);populateConfig();toast("已重新载入");}catch(e){toast(e.message,9000);}};const preview=$("[data-preview-config]");if(preview)preview.onclick=()=>previewConfig().catch(e=>toast(e.message,9000));const dl=$("[data-download-config]");if(dl)dl.onclick=()=>{try{config=collectConfig();download("site.config.json",JSON.stringify(config,null,2),"application/json");toast("配置已生成");}catch(e){toast(e.message,9000);}};const add=$("[data-hero-add]"),file=$("[data-hero-file]");if(add&&file)add.onclick=()=>file.click();if(file)file.onchange=()=>{pendingHeroFiles=[...file.files];updateHeroUploadState();toast(pendingHeroFiles.length?`已选择 ${pendingHeroFiles.length} 张图片`:"未选择图片");};const upload=$("[data-hero-upload]");if(upload)upload.onclick=()=>uploadHeroFiles().catch(e=>toast(e.message,9000));}
function updateHeroUploadState(){const btn=$("[data-hero-upload]"),file=$("[data-hero-file]"),name=$("[data-hero-file-name]");if(name)name.textContent=pendingHeroFiles.length?`已选择 ${pendingHeroFiles.length} 张图片`:"未选择图片";if(btn)btn.disabled=!pendingHeroFiles.length||!gh.token;if(file&&!pendingHeroFiles.length)file.value="";}
function rgbHex(r,g,b){return `#${[r,g,b].map(v=>Math.max(0,Math.min(255,Math.round(v))).toString(16).padStart(2,"0")).join("").toUpperCase()}`;}
async function extractImageColor(source){return new Promise(resolve=>{const img=new Image();img.onload=()=>{try{const size=48,c=document.createElement("canvas");c.width=c.height=size;const ctx=c.getContext("2d",{willReadFrequently:true});ctx.drawImage(img,0,0,size,size);const d=ctx.getImageData(0,0,size,size).data;let r=0,g=0,b=0,w=0;for(let i=0;i<d.length;i+=16){const a=d[i+3]/255;if(a<.4)continue;const rr=d[i],gg=d[i+1],bb=d[i+2],lum=(rr+gg+bb)/3;if(lum<18||lum>245)continue;const chroma=Math.max(rr,gg,bb)-Math.min(rr,gg,bb),ww=a*(1+chroma/100);r+=rr*ww;g+=gg*ww;b+=bb*ww;w+=ww;}resolve(w?rgbHex(r/w,g/w,b/w):null);}catch{resolve(null);}};img.onerror=()=>resolve(null);img.src=typeof source==="string"?source:URL.createObjectURL(source);});}
async function nextHeadNames(count){readGh();const r=await fetch(`${api("images")}?ref=${encodeURIComponent(gh.branch)}&_=${Date.now()}`,{headers:authHeaders(),cache:"no-store"});if(!r.ok){if(r.status===404)return Array.from({length:count},(_,i)=>`head${String(i+1).padStart(2,"0")}.jpeg`);throw new Error(await githubError(r,"读取 images 目录"));}const used=new Set();(await r.json()).forEach(x=>{const m=String(x.name||"").match(/^head(\d+)\.[^.]+$/i);if(m)used.add(+m[1]);});const out=[];for(let n=1;out.length<count;n++)if(!used.has(n)){out.push(`head${String(n).padStart(2,"0")}.jpeg`);used.add(n);}return out;}
function readHeroData(){const images=[...document.querySelectorAll("[data-hero-url]")].map(i=>i.value.trim()).filter(Boolean),colors={};document.querySelectorAll("[data-hero-color]").forEach(i=>{const src=i.closest(".hero-image-item")?.querySelector("[data-hero-url]")?.value.trim();if(src&&/^#[0-9a-f]{6}$/i.test(i.value))colors[src]=i.value.toUpperCase();});return{images,colors};}
function readHeroList(){return readHeroData().images;}
async function autoFillHeroColors(images,colors){for(const src of images){if(colors[src])continue;const c=await extractImageColor(src);if(c){colors[src]=c;const row=[...document.querySelectorAll("[data-hero-url]")].find(x=>x.value.trim()===src)?.closest(".hero-image-item");const input=row?.querySelector("[data-hero-color]");if(input)input.value=c;}}}
function renderHeroList(images,colors={}){const box=$("[data-hero-list]");if(!box)return;const normalized=images.map(x=>({src:String(x),color:colors[x]||""})).filter(x=>x.src);if(!normalized.length){box.innerHTML=`<div class="hero-image-item-empty">尚未配置头图。</div>`;return;}box.innerHTML=normalized.map((x,i)=>`<div class="hero-image-item" data-hero-index="${i}" draggable="true"><span class="hero-drag" title="拖动排序">☰</span><img src="${escapeHtml(x.src)}" alt="" onerror="this.style.opacity=.35"><input type="text" data-hero-url value="${escapeHtml(x.src)}"><input type="color" data-hero-color value="${/^#[0-9a-f]{6}$/i.test(x.color)?x.color:"#6750A4"}" title="编辑取色"><button class="icon-button" type="button" data-hero-remove="${i}" title="删除" aria-label="删除"><span class="symbol symbol--close" aria-hidden="true"></span></button></div>`).join("");let drag=-1;box.querySelectorAll(".hero-image-item").forEach(item=>{item.addEventListener("dragstart",()=>{drag=+item.dataset.heroIndex});item.addEventListener("dragover",e=>e.preventDefault());item.addEventListener("drop",e=>{e.preventDefault();const to=+item.dataset.heroIndex;if(drag<0||drag===to)return;const d=readHeroData(),src=d.images.splice(drag,1)[0];d.images.splice(to,0,src);renderHeroList(d.images,d.colors);});});box.querySelectorAll("[data-hero-remove]").forEach(btn=>btn.onclick=()=>{const d=readHeroData();d.images.splice(+btn.dataset.heroRemove,1);renderHeroList(d.images,d.colors);});box.querySelectorAll("[data-hero-color]").forEach(inp=>inp.onchange=()=>toast("取色已修改，请保存"));}
async function uploadHeroFiles(){if(!pendingHeroFiles.length){toast("请先选择头图");return;}readGh();if(!gh.token){toast("请先连接 GitHub");return;}const btn=$("[data-hero-upload]"),old=btn?.textContent;if(btn){btn.disabled=true;btn.textContent="上传中…";}try{const names=await nextHeadNames(pendingHeroFiles.length),d=readHeroData();for(let i=0;i<pendingHeroFiles.length;i++){const file=pendingHeroFiles[i],name=names[i],path=`images/${name}`;await githubWriteBinary(path,file,`Add hero image: ${name}`);d.images.push(path);const c=await extractImageColor(file);if(c)d.colors[path]=c;}renderHeroList(d.images,d.colors);config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update hero carousel");pendingHeroFiles=[];updateHeroUploadState();toast(`成功上传 ${names.length} 张头图`);}finally{if(btn){btn.textContent=old||"上传并保存";updateHeroUploadState();}}}
...
async function githubWrite(path,content,message){readGh();if(!gh.token)throw new Error("请先填写 GitHub PAT");const url=api(path);for(let attempt=0;attempt<5;attempt++){const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}&_=${Date.now()}-${attempt}`,{headers:{...authHeaders(),"Cache-Control":"no-cache"},cache:"no-store"});let sha;if(get.ok)sha=(await get.json()).sha;else if(get.status!==404)throw new Error(await githubError(get,`读取 ${path}`));const put=await fetch(url,{method:"PUT",cache:"no-store",headers:{...authHeaders(),"Content-Type":"application/json","Cache-Control":"no-cache"},body:JSON.stringify({message,content:b64utf8(content),branch:gh.branch,...(sha?{sha}:{})})});if(put.ok)return put.json();if(put.status===409&&attempt<4){await new Promise(r=>setTimeout(r,600*(attempt+1)));continue;}throw new Error(await githubError(put,`提交 ${path}`));}throw new Error(`提交 ${path}失败：重试次数已用尽`);}
async function githubWriteBinary(path,file,message){const bytes=new Uint8Array(await file.arrayBuffer());let bin="";for(let i=0;i<bytes.length;i+=0x8000)bin+=String.fromCharCode(...bytes.subarray(i,i+0x8000));readGh();if(!gh.token)throw new Error("请先填写 GitHub PAT");const url=api(path);for(let attempt=0;attempt<5;attempt++){const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}&_=${Date.now()}-${attempt}`,{headers:{...authHeaders(),"Cache-Control":"no-cache"},cache:"no-store"});let sha;if(get.ok)sha=(await get.json()).sha;else if(get.status!==404)throw new Error(await githubError(get,`读取 ${path}`));const put=await fetch(url,{method:"PUT",cache:"no-store",headers:{...authHeaders(),"Content-Type":"application/json","Cache-Control":"no-cache"},body:JSON.stringify({message,content:btoa(bin),branch:gh.branch,...(sha?{sha}:{})})});if(put.ok)return put.json();if(put.status===409&&attempt<4){await new Promise(r=>setTimeout(r,600*(attempt+1)));continue;}throw new Error(await githubError(put,`上传 ${path}`));}throw new Error(`上传 ${path}失败：重试次数已用尽`);}
