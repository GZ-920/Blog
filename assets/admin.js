...
function toast(msg){const el=$("[data-toast]");if(!el)return;el.textContent=msg;el.classList.add("show");clearTimeout(el._t);el._t=setTimeout(()=>el.classList.remove("show"),2200);}
...
function populateConfig(){
  ...
  const bg=config.theme?.background?.image;if(bg){const img=$("[data-bg-preview]");img.src=bg;img.hidden=false;}
}
...
async function previewConfig(){config=collectConfig();let override=null;if(pendingBgFile)override=URL.createObjectURL(pendingBgFile);await applyDynamicTheme(config,override);applyThemeMode(document.documentElement.dataset.themeMode||config.theme?.defaultMode||"light",config);if(override)setTimeout(()=>URL.revokeObjectURL(override),2000);toast("已应用预览");}
function bindConfig(){
  $("[data-reset-config]").onclick=async()=>{config=await loadConfig(true);populateConfig();toast("已重新载入");};
...
async function githubWrite(path,content,message){readGh();if(!gh.token)throw new Error("请先填写 GitHub PAT");const url=api(path);let sha;const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}`,{headers:authHeaders()});if(get.ok)sha=(await get.json()).sha;else if(get.status!==404)throw new Error(`读取 ${path} 失败：${get.status}`);const body={message,content:b64utf8(content),branch:gh.branch,...(sha?{sha}:{})};const put=await fetch(url,{method:"PUT",headers:{...authHeaders(),"Content-Type":"application/json"},body:JSON.stringify(body)});if(!put.ok){const t=await put.text();throw new Error(`提交失败 ${put.status}: ${t.slice(0,180)}`);}return put.json();}
async function githubWriteBinary(path,file,message){const bytes=new Uint8Array(await file.arrayBuffer());let bin="";for(let i=0;i<bytes.length;i+=0x8000)bin+=String.fromCharCode(...bytes.subarray(i,i+0x8000));readGh();const url=api(path);let sha;const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}`,{headers:authHeaders()});if(get.ok)sha=(await get.json()).sha;const put=await fetch(url,{method:"PUT",headers:{...authHeaders(),"Content-Type":"application/json"},body:JSON.stringify({message,content:btoa(bin),branch:gh.branch,...(sha?{sha}:{})})});if(!put.ok)throw new Error(`上传失败：${put.status}`);return put.json();}
async function testGithub(){readGh();if(!gh.owner||!gh.repo||!gh.token){toast("请填写仓库和 PAT");return;}const r=await fetch(`https://api.github.com/repos/${encodeURIComponent(gh.owner)}/${encodeURIComponent(gh.repo)}`,{headers:authHeaders()});if(!r.ok){toast(`连接失败：${r.status}`);return;}$("[data-admin-status]").textContent="GitHub 已连接";$("[data-push-config]").disabled=false;$("[data-push-post]").disabled=false;$("[data-upload-background]").disabled=!pendingBgFile;toast("连接成功");}
function bindGithub(){
  $("[data-gh-test]").onclick=testGithub;
  $("[data-push-config]").onclick=async()=>{try{config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update site config");toast("配置已提交");}catch(e){toast(e.message);}};
  $("[data-push-post]").onclick=async()=>{try{if(!saveSession())return;await githubWrite(`posts/${current.file}.md`,currentMd,`Update post: ${current.title}`);await githubWrite("posts/posts.json",JSON.stringify(posts,null,2),"Update posts index");toast("文章已提交");}catch(e){toast(e.message);}};
  $("[data-upload-background]").onclick=async()=>{if(!pendingBgFile)return;try{const ext=(pendingBgFile.name.split(".").pop()||"jpg").toLowerCase();const path=`images/background.${ext}`;await githubWriteBinary(path,pendingBgFile,"Update blog background");$("[name='theme.background.image']").value=path;config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update background config");toast("背景已上传并更新配置");}catch(e){toast(e.message);}};
}
