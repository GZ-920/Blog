...
async function githubWrite(path,content,message){
  ...
  for(let attempt=0;attempt<4;attempt++){
    let sha;
    const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}&_=${Date.now()}-${attempt}`,{headers:{...authHeaders(),"Cache-Control":"no-cache"},cache:"no-store"});
    if(get.ok)sha=(await get.json()).sha;
    else if(get.status!==404)throw new Error(await githubError(get,`读取 ${path}`));
    const put=await fetch(url,{method:"PUT",cache:"no-store",headers:{...authHeaders(),"Content-Type":"application/json","Cache-Control":"no-cache"},body:JSON.stringify({message,content:b64utf8(content),branch:gh.branch,...(sha?{sha}:{})})});
    if(put.ok)return put.json();
    if(put.status===409&&attempt<3){await new Promise(r=>setTimeout(r,350*(attempt+1)));continue;}
...
async function githubWriteBinary(path,file,message){
  ...
  for(let attempt=0;attempt<4;attempt++){
    ...
    const put=await fetch(url,{method:"PUT",cache:"no-store",headers:{...authHeaders(),"Content-Type":"application/json","Cache-Control":"no-cache"},body:JSON.stringify({message,content:btoa(bin),branch:gh.branch,...(sha?{sha}:{})})});
    if(put.ok)return put.json();
    if(put.status===409&&attempt<3){await new Promise(r=>setTimeout(r,350*(attempt+1)));continue;}
