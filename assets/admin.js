...
function bindConfig(){
  ...
  const heroUpload=$("[data-hero-upload]");
  const heroFile=$("[data-hero-file]");
  if(heroUpload&&heroFile)heroUpload.onclick=e=>{if(e.target!==heroFile)heroFile.click();};
  if(heroFile)heroFile.onchange=()=>handleHeroUpload().catch(e=>toast(e.message,9000));
}
