...
function bindConfig(){
  ...
  $("[data-hero-add]").onclick=()=>{const list=readHeroList();list.push("images/hero-new.jpg");renderHeroList(list);};
  $("[data-hero-add-bg]").onclick=()=>{const bg=$("[name='theme.background.image']").value.trim()||config.theme?.background?.image;if(!bg){toast("请先设置背景图");return;}const list=readHeroList();if(!list.includes(bg))list.unshift(bg);renderHeroList(list);};
...
function renderHeroList(images){
  ...
  box.innerHTML=images.map((src,i)=>`<div class="hero-image-item" data-hero-index="${i}"><img src="${escapeHtml(src)}" alt=""><input type="text" data-hero-url value="${escapeHtml(src)}" placeholder="images/hero.jpg" /><button class="icon-button" type="button" data-hero-remove="${i}" title="移除" aria-label="移除"><span class="symbol symbol--close" aria-hidden="true"></span></button></div>`).join("");
  box.querySelectorAll("[data-hero-remove]").forEach(btn=>btn.onclick=()=>{const list=readHeroList();list.splice(Number(btn.dataset.heroRemove),1);renderHeroList(list);});
  box.querySelectorAll("[data-hero-url]").forEach(inp=>inp.oninput=()=>{const list=readHeroList();const i=Number(inp.closest(".hero-image-item").dataset.heroIndex);list[i]=inp.value.trim();});
}
function readHeroList(){return[...document.querySelectorAll("[data-hero-url]")].map(i=>i.value.trim()).filter(Boolean);}
...
async function testGithub(silent=false){readGh();if(!gh.owner||!gh.repo||!gh.token){toast("请填写仓库和 PAT");return;}const user=await fetch("https://api.github.com/user",{headers:authHeaders()});if(!user.ok){toast(await githubError(user,"PAT 认证"),9000);return;}const me=await user.json();const r=await fetch(`https://api.github.com/repos/${encodeURIComponent(gh.owner)}/${encodeURIComponent(gh.repo)}`,{headers:authHeaders()});if(!r.ok){toast(await githubError(r,"访问仓库"),9000);return;}$("[data-admin-status]").textContent=`已认证：${me.login}`;$("[data-push-config]").disabled=false;$("[data-push-post]").disabled=false;$("[data-save-giscus]").disabled=false;$("[data-save-password]").disabled=false;$("[data-save-hero]").disabled=false;$("[data-upload-background]").disabled=!pendingBgFile;if($("[data-gh-remember]").checked){localStorage.setItem(GH_STORAGE_KEY,JSON.stringify({owner:gh.owner,repo:gh.repo,branch:gh.branch,token:gh.token}));localStorage.setItem(GH_REMEMBER_KEY,"1");}else{localStorage.removeItem(GH_STORAGE_KEY);localStorage.removeItem(GH_REMEMBER_KEY);}if(!silent)toast(`PAT 认证成功：${me.login}。注意：公开仓库读取成功不代表该 PAT 已获写权限。`,6500);}
function bindGithub(){
  $("[data-gh-test]").onclick=()=>testGithub(false);
  $("[data-save-giscus]").onclick=async()=>{try{config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update Giscus config");$("[data-giscus-state]").textContent="已保存";toast("Giscus 配置已保存");}catch(e){toast(e.message,9000);}};
  $("[data-save-hero]").onclick=async()=>{try{config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update hero carousel");toast("头图轮播已保存");}catch(e){toast(e.message,9000);}};
