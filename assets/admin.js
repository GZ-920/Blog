...
function fillEditor(){if(!current)return;$("[data-editor-heading]").textContent=current.title||"新文章";$("[data-post-file]").value=current.file||"";$("[data-post-cat]").value=current.cat||"essay";$("[data-post-title]").value=current.title||"";$("[data-post-desc]").value=current.desc||"";$("[data-post-category]").value=current.category||"";$("[data-post-date]").value=current.date||new Date().toISOString().slice(0,10);$("[data-post-time]").value=current.time||"5 min";$("[data-post-cover-home]").value=current.coverHome||current.cover||"";$("[data-post-cover-article]").value=current.coverArticle||current.cover||"";$("[data-post-featured]").checked=!!current.featured;$("[data-post-tags]").value=asList(current.tags).join(", ");$("[data-markdown]").value=currentMd;renderPreview();}
function collectPost(){const home=$("[data-post-cover-home]").value.trim(),article=$("[data-post-cover-article]").value.trim(),same=home||article;const coverHome=home||same,coverArticle=article||same;return{file:$("[data-post-file]").value.trim(),cat:$("[data-post-cat]").value,category:$("[data-post-category]").value.trim(),log:current?.log||"POST",title:$("[data-post-title]").value.trim(),desc:$("[data-post-desc]").value.trim(),date:$("[data-post-date]").value,time:$("[data-post-time]").value.trim(),coverHome,coverArticle,cover:coverHome,featured:$("[data-post-featured]").checked,tags:asList($("[data-post-tags]").value)};}
