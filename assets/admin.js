import { loadConfig, initTheme, applyThemeMode, escapeHtml } from "./shared.js";
...
function toast(msg){ const t=$("[data-toast]"); t.textContent=msg; t.classList.add("is-visible"); clearTimeout(toast._timer); toast._timer=setTimeout(()=>t.classList.remove("is-visible"),2600); }
...
function fillConfigForm(){
  $$('#config-form [name]').forEach(input=>{
    ...
    else if(input.name==='home.chips') input.value=asList(v).join(', ');
    else input.value=v??'';
  });
...
function readConfigForm(){
  $$('#config-form [name]').forEach(input=>{
    ...
    if(input.name==='home.chips') v=String(v).split(/[,，]/).map(s=>s.trim()).filter(Boolean);
...
async function loadPosts(){
  const r=await fetch(`posts/posts.json?t=${Date.now()}`,{cache:'no-store'}); if(!r.ok)throw new Error(`posts.json HTTP ${r.status}`);posts=await r.json();renderPostList();if(posts.length)await selectPost(0);
}
...
async function selectPost(i){
  ...
  $('[data-post-file]').value=p.file||'';$('[data-post-cat]').value=p.cat||'essay';$('[data-post-title]').value=p.title||'';$('[data-post-desc]').value=p.desc||'';$('[data-post-category]').value=p.category||'';$('[data-post-date]').value=p.date||'';$('[data-post-time]').value=p.time||'';$('[data-post-cover]').value=p.cover||'';$('[data-post-featured]').checked=!!p.featured;$('[data-post-tags]').value=(p.tags||[]).join(', ');
...
async function getGithubSha(path){ const r=await fetch(`${ghUrl(`contents/${path.split('/').map(encodeURIComponent).join('/')}`)}?ref=${encodeURIComponent(gh.branch)}`,{headers:ghHeaders()});if(r.status===404)return null;if(!r.ok)throw new Error(`读取 ${path} 失败：HTTP ${r.status}`);return (await r.json()).sha; }
async function putGithubFile(path,base64,message){ const sha=await getGithubSha(path); const body={message,content:base64,branch:gh.branch};if(sha)body.sha=sha;const r=await fetch(ghUrl(`contents/${path.split('/').map(encodeURIComponent).join('/')}`),{method:'PUT',headers:{...ghHeaders(),'Content-Type':'application/json'},body:JSON.stringify(body)});if(!r.ok){const d=await r.json().catch(()=>({}));throw new Error(d.message||`HTTP ${r.status}`);}return r.json(); }
async function testGithub(){ syncGhState(); if(!gh.owner||!gh.repo||!gh.token){toast('请填写 Owner、Repository 和 PAT');return;}const b=$('[data-gh-test]');b.disabled=true;try{const r=await fetch(`${ghUrl('contents')}?ref=${encodeURIComponent(gh.branch)}`,{headers:ghHeaders()});if(!r.ok)throw new Error(`HTTP ${r.status}`);gh.connected=true;const status=$('[data-admin-status]');status.textContent=`已连接 ${gh.owner}/${gh.repo}`;status.classList.add('is-ok');$('[data-push-config]').disabled=false;$('[data-push-post]').disabled=false;$('[data-upload-background]').disabled=!backgroundFile;toast('GitHub 连接成功');}catch(e){gh.connected=false;toast(`连接失败：${e.message}`);}finally{b.disabled=false;} }
