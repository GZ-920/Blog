...
async function uploadHeroFiles(){
  ...
  try{
    ...
    const state=$("[data-hero-upload-state]"),btn=$("[data-hero-upload-save]");if(btn)btn.disabled=true;
    const existing=readHeroList();let next=1;
    for(const f of pendingHeroFiles){
      while(existing.some(x=>/^images\/head\d+\./i.test(x.src)&&Number((x.src.match(/head(\d+)\./i)||[])[1])===next))next++;
      const ext=extensionFor(f),path=`images/head${String(next).padStart(2,"0")}.${ext}`;
      await githubWriteBinary(path,f,`Add hero image ${path}`);
      const color=await autoColor(f);existing.push({src:path,color});next++;
      if(state)state.textContent=`已上传 ${pendingHeroFiles.indexOf(f)+1}/${pendingHeroFiles.length} 张`;
    }
    renderHeroList(existing);config=collectConfig();await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Update hero carousel");
    pendingHeroFiles=[];if($("[data-hero-file]"))$("[data-hero-file]").value="";if(state)state.textContent="上传并保存成功";toast("头图上传并保存成功",5000);
  }catch(e){const state=$("[data-hero-upload-state]");if(state)state.textContent="上传失败";toast(e.message,9000);}finally{const btn=$("[data-hero-upload-save]");if(btn)btn.disabled=false;}
}
...
async function githubWrite(path,content,message){
  readGh();if(!gh.token)throw new Error("请先填写 GitHub PAT");const url=api(path);
  for(let attempt=0;attempt<4;attempt++){
    let sha;const get=await fetch(`${url}?ref=${encodeURIComponent(gh.branch)}&_=${Date.now()}`,{headers:{...authHeaders(),"Cache-Control":"no-cache"},cache:"no-store"});
    ...
    if(put.status!==409||attempt===3)throw new Error(await githubError(put,`提交 ${path}`));
...
async function githubWriteBinary(path,file,message){
  ...
  for(let attempt=0;attempt<4;attempt++){
    ...
    if(put.ok)return put.json();
    if(put.status!==409||attempt===3)throw new Error(await githubError(put,`上传 ${path}`));
    await new Promise(r=>setTimeout(r,500*(attempt+1)));
  }
}
