...
async function autoColorFromUrl(url){try{const img=new Image();img.crossOrigin="anonymous";await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=url;});const c=document.createElement("canvas"),ctx=c.getContext("2d",{willReadFrequently:true});const size=40;c.width=c.height=size;ctx.drawImage(img,0,0,size,size);const d=ctx.getImageData(0,0,size,size).data;let r=0,g=0,b=0,n=0;for(let i=0;i<d.length;i+=4){if(d[i+3]<180)continue;r+=d[i];g+=d[i+1];b+=d[i+2];n++;}if(!n)return"#c0a089";return`#${[r/n,g/n,b/n].map(v=>Math.round(v).toString(16).padStart(2,"0")).join("")}`;}catch{return"#c0a089";}}
...
async function addHeroLink(){
  const input=$("[data-hero-link]");const url=(input?.value||"").trim();
  if(!url){toast("请先填写图片直链");return;}
  const state=$("[data-hero-upload-state]");if(state)state.textContent="取色中…";
  const color=await autoColorFromUrl(url);
  const list=readHeroList();list.push({src:url,color});renderHeroList(list);
  input.value="";if(state)state.textContent="已添加，别忘了点击“保存轮播设置”";
  toast("已添加直链图片，记得保存轮播设置以提交到 GitHub");
}
