...
function populateConfig(){
  ...
  const ib=hc.imageBed||{};$("[data-imagebed-enabled]").checked=!!ib.enabled;$("[data-imagebed-url]").value=ib.uploadUrl||"";$("[data-imagebed-field]").value=ib.fieldName||"file";$("[data-imagebed-auth]").value=ib.authHeader||"";$("[data-imagebed-path]").value=ib.urlPath||"url";
}
function collectConfig(){
  ...
  out.theme={...(out.theme||{}),heroCarousel:{enabled:$("[data-hero-enabled]").checked,autoplay:$("[data-hero-autoplay]").checked,interval:Math.max(1000,Number($("[data-hero-interval]").value)||3000),images:readHeroList(),imageBed:readImageBed()}};
...
function bindConfig(){
  const reset=$("[data-reset-config]"),preview=$("[data-preview-config]"),downloadBtn=$("[data-download-config]"),add=$("[data-hero-add]"),file=$("[data-hero-file]"),choose=$("[data-hero-upload]"),saveUpload=$("[data-hero-upload-save]");
  ...
  if(file)file.onchange=()=>{pendingHeroFiles=[...file.files];if(!pendingHeroFiles.length){toast("未选择图片");return;}const state=$("[data-hero-upload-state]");if(state)state.textContent=`已选择 ${pendingHeroFiles.length} 张图片`;toast(`已选择 ${pendingHeroFiles.length} 张头图`);};
...
function readImageBed(){return{enabled:$("[data-imagebed-enabled]").checked,uploadUrl:$("[data-imagebed-url]").value.trim(),fieldName:$("[data-imagebed-field]").value.trim()||"file",authHeader:$("[data-imagebed-auth]").value.trim(),urlPath:$("[data-imagebed-path]").value.trim()||"url"};}
function deepGetPath(obj,path){return path.split(".").reduce((v,k)=>v&&typeof v==="object"?v[k]:undefined,obj);}
async function uploadToImageBed(file){
  const ib=readImageBed();
  if(!ib.uploadUrl)throw new Error("请先填写图床上传接口地址");
  const fd=new FormData();fd.append(ib.fieldName,file);
  const headers={};
  if(ib.authHeader){const idx=ib.authHeader.indexOf(":");if(idx>0)headers[ib.authHeader.slice(0,idx).trim()]=ib.authHeader.slice(idx+1).trim();}
  let r;try{r=await fetch(ib.uploadUrl,{method:"POST",headers,body:fd});}catch(e){throw new Error(`图床上传失败：${e?.message||"网络错误"}`);}
  if(!r.ok)throw new Error(`图床上传失败 HTTP ${r.status}`);
  const data=await r.json().catch(()=>null);
  let url=data?typeof deepGetPath(data,ib.urlPath)==="string"?deepGetPath(data,ib.urlPath):null:null;
  if(!url&&data){for(const p of["url","data.url","data.link","image.url","links.url","result.url"]){const v=deepGetPath(data,p);if(typeof v==="string"){url=v;break;}}}
  if(!url)throw new Error("图床返回结果中未找到图片地址，请检查“返回结果中的图片地址字段”填写是否正确");
  return url;
}
async function uploadHeroFiles(){
  ...
  const ib=readImageBed();
  try{
    if(btn)btn.disabled=true;const existing=readHeroList();
    if(ib.enabled){
      for(const f of pendingHeroFiles){
        if(state)state.textContent=`上传中 ${existing.length+1}/${existing.length+pendingHeroFiles.length}…`;
        const [url,color]=await Promise.all([uploadToImageBed(f),autoColor(f)]);
        existing.push({src:url,color});
      }
      config.theme={...(config.theme||{}),heroCarousel:{enabled:$("[data-hero-enabled]").checked,autoplay:$("[data-hero-autoplay]").checked,interval:Math.max(1000,Number($("[data-hero-interval]").value)||3000),images:existing,imageBed:ib}};
      readGh();if(!gh.token){renderHeroList(existing);pendingHeroFiles=[];if($("[data-hero-file]"))$("[data-hero-file]").value="";if(state)state.textContent="已上传（未连接 GitHub，配置未提交）";toast("图片已上传到图床，但未连接 GitHub，请手动保存配置",6000);return;}
      await githubWrite("assets/site.config.json",JSON.stringify(config,null,2),"Add/update hero carousel images (external image bed)");
      renderHeroList(existing);pendingHeroFiles=[];if($("[data-hero-file]"))$("[data-hero-file]").value="";if(state)state.textContent="上传并保存成功";toast("图片已上传到图床并保存成功",5000);
      return;
    }
    readGh();if(!gh.token){toast("请先连接 GitHub");return;}
    let next=1;const files=[];
    for(const f of pendingHeroFiles){
      while(existing.some(x=>/^images\/head\d+\./i.test(x.src)&&Number((x.src.match(/head(\d+)\./i)||[])[1])===next))next++;
      const ext=extensionFor(f),path=`images/head${String(next).padStart(2,"0")}.${ext}`;const bytes=new Uint8Array(await f.arrayBuffer());let bin="";for(let i=0;i<bytes.length;i+=0x8000)bin+=String.fromCharCode(...bytes.subarray(i,i+0x8000));
      files.push({path,content:btoa(bin)});const color=await autoColor(f);existing.push({src:path,color});next++;
      if(state)state.textContent=`准备 ${files.length}/${pendingHeroFiles.length} 张`;
    }
    const nextConfig=clone(config);nextConfig.theme={...(nextConfig.theme||{}),heroCarousel:{enabled:$("[data-hero-enabled]").checked,autoplay:$("[data-hero-autoplay]").checked,interval:Math.max(1000,Number($("[data-hero-interval]").value)||3000),images:existing,imageBed:ib}};
    files.push({path:"assets/site.config.json",content:b64utf8(JSON.stringify(nextConfig,null,2))});
    await githubCommit(files,"Add/update hero carousel images");config=nextConfig;renderHeroList(existing);pendingHeroFiles=[];if($("[data-hero-file]"))$("[data-hero-file]").value="";if(state)state.textContent="上传并保存成功";toast("头图上传并保存成功",5000);
  }catch(e){if(state)state.textContent="上传失败";toast(e.message,9000);}finally{if(btn)btn.disabled=false;}
}
...
async function autoColor(file){try{const url=URL.createObjectURL(file);const img=new Image();await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=url;});const c=document.createElement("canvas"),ctx=c.getContext("2d",{willReadFrequently:true});const size=40;c.width=c.height=size;ctx.drawImage(img,0,0,size,size);const d=ctx.getImageData(0,0,size,size).data;let r=0,g=0,b=0,n=0;for(let i=0;i<d.length;i+=4){if(d[i+3]<180)continue;r+=d[i];g+=d[i+1];b+=d[i+2];n++;}URL.revokeObjectURL(url);if(!n)return"#c0a089";return`#${[r/n,g/n,b/n].map(v=>Math.round(v).toString(16).padStart(2,"0")).join("")}`;}catch{return"#c0a089";}}
...
function bindGithub(){
  ...
  $("[data-push-post]").onclick=async()=>{try{if(!saveSession())return;await githubCommit([{path:`posts/${current.file}.md`,content:b64utf8(currentMd)},{path:"posts/posts.json",content:b64utf8(JSON.stringify(posts,null,2))}],`Update post: ${current.title}`);toast("文章已提交");}catch(e){toast(e.message,9000);}};
}
