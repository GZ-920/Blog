...
function initHeroCarousel(config){
  ...
  const images=asList(cf.images).filter(Boolean);
  ...
  const applyThemeFor=i=>{
    const target=images[i];if(!target||!window.__applyDynamicTheme)return;
    const manual=cf.colors?.[target];
    window.__applyDynamicTheme({...config,theme:{...config.theme,seedColor:manual||config.theme?.seedColor,source:manual?"manual":"background",background:{...config.theme.background,image:target}}},null,{skipOverlay:true});
  };
  const go=next=>{
