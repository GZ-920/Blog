...
function postMatches(p){
  ...
  const hay=[p.title,p.desc,p.category,p.log,...(p.tags||[])].join(" ").toLowerCase(); return hay.includes(query);
}
  ...
  c.innerHTML=posts.map(p=>{
    ...
    return `<a class="post-card ${p.featured?"featured":""}" href="${href}" data-reveal>
      ...
      <div class="post-inner">
        ...
        <div class="post-footer"><span>${escapeHtml(p.date||"")} · ${escapeHtml(p.time||"")}</span><span class="tag-mini">${(p.tags||[]).slice(0,2).map(t=>`<span>${escapeHtml(t)}</span>`).join("")}</span></div>
      </div></a>`;
  }).join("");
