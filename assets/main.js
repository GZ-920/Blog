import { loadConfig, bootCommon, escapeHtml, escapeAttr, toStringArray } from "./shared.js";
...
function renderHome(config){
  ...
  const chips=document.querySelector("[data-home-chips]"); if(chips) chips.innerHTML=toStringArray(config.home?.chips).map(x=>`<span class="chip">#${escapeHtml(x)}#</span>`).join("");
  renderHomeNotice(config);
}
function renderHomeNotice(config){
  const wrap=document.querySelector("[data-home-notice-wrap]"); if(!wrap) return;
  const notice=config.home?.notice||{};
  const text=String(notice.text||"").trim();
  const key=`chuyuan-home-notice-dismissed:${text}`;
  const dismissed=notice.dismissible!==false && sessionStorage.getItem(key)==="1";
  if(!notice.enabled || !text || dismissed){ wrap.hidden=true; return; }
  const textEl=wrap.querySelector("[data-home-notice-text]"); if(textEl) textEl.textContent=text;
  const link=wrap.querySelector("[data-home-notice-link]");
  const url=String(notice.url||"").trim(), linkText=String(notice.linkText||"").trim();
  if(link){
    if(url&&linkText){link.href=url;link.textContent=linkText;link.hidden=false;}
    else{link.removeAttribute("href");link.textContent="";link.hidden=true;}
  }
  const close=wrap.querySelector("[data-home-notice-close]");
  if(close){
    close.hidden=notice.dismissible===false;
    close.onclick=()=>{sessionStorage.setItem(key,"1");wrap.hidden=true;};
  }
  wrap.hidden=false;
}
...
function postMatches(p){
  ...
  const hay=[p.title,p.desc,p.category,p.log,...toStringArray(p.tags)].join(" ").toLowerCase(); return hay.includes(query);
}
  ...
  c.innerHTML=posts.map(p=>{
    ...
    return `<a class="post-card ${p.featured?"featured":""}" href="${href}" data-reveal>
      ...
      <div class="post-inner">
        ...
        <div class="post-footer"><span>${escapeHtml(p.date||"")} · ${escapeHtml(p.time||"")}</span><span class="tag-mini">${toStringArray(p.tags).slice(0,2).map(t=>`<span>${escapeHtml(t)}</span>`).join("")}</span></div>
      </div></a>`;
  }).join("");
...
async function loadPosts(){
  const c=document.getElementById("post-container"); if(!c) return;
  try{
    const r=await fetch("posts/posts.json",{cache:"no-store"}); if(!r.ok) throw new Error(`HTTP ${r.status}`); allPosts=await r.json(); if(!Array.isArray(allPosts)) throw new Error("posts.json 顶层必须是数组");
...
(async()=>{
  try{
    const config=await loadConfig(); renderHome(config); await bootCommon(config); initSearch(config); await loadPosts();
