...
function initHeroCarousel(config){
  ...
  const items=asList(cf.images).map(x=>typeof x==="string"?{src:x,color:""}:{src:String(x?.src||x?.url||""),color:String(x?.color||"")}).filter(x=>x.src);
  ...
  const applyThemeFor=i=>{
    const item=items[i];if(!item||!window.__applyDynamicTheme)return;
    const theme=item.color?{...config.theme,seedColor:item.color,dynamicColor:false}:{...config.theme,seedColor:config.theme?.seedColor||"#6750A4",dynamicColor:true,source:"background",background:{...config.theme?.background,image:item.src}};
...
(async()=>{try{const config=await loadConfig();const first=config.theme?.heroCarousel?.images?.[0];const firstItem=typeof first==="string"?{src:first,color:""}:first;if(firstItem?.color)config.theme={...config.theme,seedColor:firstItem.color,dynamicColor:false};else if(firstItem?.src)config.theme={...config.theme,dynamicColor:true,source:"background",background:{...config.theme?.background,image:firstItem.src}};renderHome(config);renderBanner(config);await bootCommon(config);initSearch(config);await loadPosts();initHeroCarousel(config);}catch(e){console.error(e);document.body.insertAdjacentHTML("afterbegin",`<div style="padding:12px;text-align:center;background:#ba1a1a;color:white">配置加载失败：${escapeHtml(e.message)}</div>`);}})();
