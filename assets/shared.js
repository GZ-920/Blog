...
export async function bootCommon(config,imageOverride=null){
  ...
  applySiteChrome(config);await initTheme(config,imageOverride);initReveal();initBackToTop(config.features?.backToTop!==false);registerPwa(config.features?.pwa!==false);
