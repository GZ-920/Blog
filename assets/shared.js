...
export function applySiteChrome(config) {
  ...
  const renderLinks = (container, buttonStyle=false) => {
    if (!container) return;
    container.innerHTML = (config.social || []).map(x => `<a class="${buttonStyle ? "tonal-button" : ""}" href="${escapeAttr(x.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(x.label)}</a>`).join("");
  };
