...
export function applySiteChrome(config){
  setText("[data-site-name]",config.site?.name||"初源博文");setText("[data-site-tagline]",config.site?.tagline||"SEEKING THE TRUTH");setText("[data-copyright]",config.site?.copyright||"");
...
function parseHex(hex){
  ...
  const m=value.match(/^#([\da-f]{6})$/i);if(!m)return null;
...
export async function initTheme(config,imageOverride=null){
  ...
  localStorage.setItem(THEME_KEY,mode);applyThemeMode(mode,config);
  document.querySelectorAll("[data-theme-toggle]").forEach(btn=>btn.addEventListener("click",()=>{const cur=normalizeThemeMode(localStorage.getItem(THEME_KEY),config),next=cur==="dark"?"light":"dark";localStorage.setItem(THEME_KEY,next);applyThemeMode(next,config);}));
}
...
export async function bootCommon(config,imageOverride=null){applySiteChrome(config);await initTheme(config,imageOverride);initReveal();initBackToTop(config.features?.backToTop!==false);registerPwa(config.features?.pwa!==false);}
